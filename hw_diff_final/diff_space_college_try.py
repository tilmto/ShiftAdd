...
print('Save trial info at', args.save_path)
...
# state = {}
# state['alpha'] = alpha
# state['beta_list'] = beta_list
# state['gamma_list'] = gamma_list
# state['metric_list'] = metric_list
# state['stop_epoch_list'] = stop_epoch_list

# torch.save(state, 'hw_param_%.3f' % metric)

if args.final_infer:
    print('Start Inference...')

    if args.fix_pe_array:
        pe_array = get_fixed_pe_array(num_pe_array, np_random_seed=args.random_seed)
    else:
        pe_array = alpha.argmax()


    if args.fix_loop_order:
        tmp_list = get_fixed_loop_order(num_loop_order, np_random_seed=args.random_seed)
    else:
        tmp_list = []
        for i in range(num_loop_order):
            order_list = beta_list[i].argsort(descending=True)

            for order in order_list:
                if order not in tmp_list:
                    break
    ...
    if args.fix_tiling:
        tiling_scheme = get_fixed_tiling(partitioned_choices, np_random_seed=args.random_seed)
    else:
        tiling_scheme = []
        for i in range(len(gamma_list[pe_array])):
            factor = gamma_list[pe_array][i].argmax()
            tiling_scheme.append(factor)

    tiling_scheme_string = tiling_translation(tiling_scheme,tiling_pool,alloc_slots,pe_array,space_partition)

    metric = -life_eval(tiling_scheme_string, 1, tmp_hw_spec, df_order=lp_order_string)

    print('Inference Metric:', metric)